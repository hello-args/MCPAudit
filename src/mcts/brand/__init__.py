...
LOGO_REPORT_PATH = BRAND_DIR / "logo-report.png"  # icon-only crop for small HTML embeds


def logo_data_uri(*, for_report: bool = True) -> str:
    """Return a data URI for embedding the logo in HTML.

    Reports use ``logo-report.png`` (shield icon only) so the sidebar mark stays
    legible at 44×44px. Terminals and large displays use the full ``logo.png``.
    """
